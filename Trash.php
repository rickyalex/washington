...
<body style="text-align:left">
	...
	<div id="content">
		...
		<div style="padding:10px">
			...
			?>
			<table id="trashgrid" class="easyui-datagrid" style="width:auto;height:auto"
					title="Trash" 
					url="php/gettrash.php"
					...
					data-options="
								...
								onClickRow: function(index,row){ 
																 ...
																 addTab('Trash: '+row.id_transaction, row.url);
																 $('#trashgrid').datagrid('clearSelections');
																}
								"> 						
				<thead>
					<tr>
			...
			<br/>
			<a href="#" class="easyui-linkbutton" onclick="deleteTrash();" data-options="plain:false,iconCls:'icon-trash'">Delete</a> 
				<script type="text/javascript">  
					function deleteTrash(){  
						var ids = [];  
						var submit = false;
						//var rows = $('#trashgrid').datagrid('getSelections');  
						var rows = $('#trashgrid').datagrid('getChecked');  
						...
						if(submit){
							$.post("data/deletemessage.php?mode=delete", {IDS:ids.join(',')})
								.done(function(data) {
								if(data!='0'){
								   $('#trashgrid').datagrid('reload');
								   $.messager.alert("Info", data + " row(s) deleted" );
								}
							});		
						}
					}  

...
function SaveInsert() {
    ...
    $uniq_id    = isset($_GET['uniq_id']) ? $_GET['uniq_id'] : '';
    if($uniq_id==''){
        $message = "Penarikan Barang No. <font color='Red'>".$id_transaction."</font> Uniq ID not found !";
        $icon = "<img src='./css/images/info2.jpeg' width='30' height='30'>";
        header('Location: ../Message.php?msg='.$message.'&icon='.$icon);
        die;
    }
    ...
    //die;
//    $SelectSQL = "SELECT * FROM TEMP_Header_FG WHERE UNIQ_ID=?";
//    $param = array($uniq_id);
//    $rs = $conn->Execute($SelectSQL,$param);
//    
//    while(!$rs->EOF){
//        array($arr, array(
...
//    die;
    ...
    $record['kategori_barang']	  = $kategori_barang;
    $record['nama_customer']      = $nama_customer;
    $record['alamat_customer']    = $alamat_customer;    
    $record['telepon_customer']   = $telepon_customer; 
    $record['cp_customer']        = $cp_customer;
    $record['cost_center_penanggung'] 	    = $cost_center_penanggung;
    $record['jo']                 = $jo;  
    $record['tgl_request_penarikan']          = $tgl_request_penarikan;
    
    
    $sql_update = " SELECT *
                        FROM Penarikan..Detail_Barang 
                        WHERE uniq_id=?";                   
    ...
    $detail = array(); # Initialize an array to hold the record data to insert
    $detail['nama_barang']      = $rs2->fields['nama_barang'];
    $detail['panjang']          = $rs2->fields['panjang'];
    $detail['lebar']            = $rs2->fields['lebar'];    
    $detail['tinggi']           = $rs2->fields['tinggi'];
    $detail['berat']            = $rs2->fields['berat'];
    $detail['jumlah'] 	        = $rs2->fields['jumlah'];
    $detail['remark']           = $rs2->fields['remark'];
    ...
    $detail_SQL = $conn->GetUpdateSQL($rs2, $detail);              //update
...
//    }
    ...
    if($approver->id_user1 == $approver->id_user2){
        $workflow       = new Workflow_Class($id_workflow, $next_state, 'AP');  
        $next_status    = $workflow->id_status;
        $next_state     = $workflow->next_state;      
    }
...
//        die;
	...
	$mail_subject   = "[TRIAL] Sistem Penarikan Barang ".$id_transaction." Need your Approval";
	$mail_icon      = "11";
...
//    }           
    ...
    $outstanding_task['from'] 	            = $from_user;
    $outstanding_task['revision']           = $revision;
    $outstanding_task['id_notification']    = 'CDSRG';
...
//    }
    ...
    if($ok){
        ...
        //Execute LN_Mail
        $conn->BeginTrans();
        $exec = $conn->Execute($insertLN_SQL);
            if (!$exec) {
                $conn->RollbackTrans();  
            } else {
                $conn->CommitTrans();
...
//	}
    if($next_status=='PCD'){
        $mail_subject   = "[TRIAL] Sistem Penarikan Barang ".$id_transaction." Need your Verification";
    }elseif($next_status=='VCD'){
		$mail_subject   = "[TRIAL] Sistem Penarikan Barang ".$id_transaction." Need your Closing";
	}
...
//        if($next_status == 'PGA'){
//            $message = "Penarikan Barang No. <font color='Red'>".$id_transaction."</font> has been POST to GA";
//        }elseif($next_status == 'VGA'){
...
//    } 
    ...
    if($ok){
        ...
        $mail_subject   = "[TRIAL] Sistem Penarikan Barang ".$id_transaction." Need your Approve";
        $mail_icon      = "11";
...
//    }
    ...
    $mail_subject   = "[TRIAL] Sistem Penarikan Barang ".$id_transaction." Rejected!";
    $mail_icon      = "11";
